...
export class BlockRewardEvent {
  constructor(
    public block_number: uint64,
    public block_reward: uint128,
    public gas_fees: uint128,
    public miner: AccountAddress
  ) {}
...
export class DepositEvent {
  constructor(
    ...
    public token_code: TokenCode,
    public metadata: Seq<uint8>
  ) {}
...
export class VoteChangedEvent {
  constructor(
    public proposal_id: uint64,
    public proposer: AccountAddress,
    public voter: AccountAddress,
    public agree: bool,
    public vote: uint128
  ) {}
...
export class Helpers {
  ...
  static serializeVectorU128(
    value: Seq<uint128>,
    serializer: Serializer
  ): void {
    serializer.serializeLen(value.length);
    value.forEach((item: uint128) => {
      serializer.serializeU128(item);
    });
  }

  static deserializeVectorU128(deserializer: Deserializer): Seq<uint128> {
    const length = deserializer.deserializeLen();
    const list: Seq<uint128> = [];
    for (let i = 0; i < length; i++) {
  ...
  static serializeVectorAccountAddress(
    value: Seq<AccountAddress>,
    serializer: Serializer
  ): void {
    serializer.serializeLen(value.length);
  ...
  static deserializeVectorAccountAddress(
    deserializer: Deserializer
  ): Seq<AccountAddress> {
    const length = deserializer.deserializeLen();
