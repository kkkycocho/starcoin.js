...
test('encodeScriptFunctionByResolve', async () => {
  ...
  const args = ['0x1df9157f14b0041eed18dcc56520d829', 3185136000000000];
  const nodeUrl = 'https://barnard-seed.starcoin.org';
  const scriptFunction = await encodeScriptFunctionByResolve(
    functionId,
    typeArgs,
    args,
    nodeUrl
...
test('encodeScriptFunctionByResolve2', async () => {
  ...
  const functionId =
    '0xb987F1aB0D7879b2aB421b98f96eFb44::MerkleDistributorScript::claim_script';
  const typeArgs = ['0x1::STC::STC'];
  const args = [
    record.ownerAddress,
    record.airDropId,
  ...
  // console.log({ payloadInHex })
  const hexExpected =
    '';
  expect(payloadInHex).toBe(hexExpected);
}, 10000);
...
test('encodeScriptFunctionByResolve3', async () => {
  ...
  const hexExpected =
    '';
  expect(payloadInHex).toBe(hexExpected);
}, 10000);
...
test('encodeScriptFunctionByResolve4', async () => {
  ...
  const senderPrivateKeyHex =
    'ac4e1d2b8ef0503ce5a5156d075c3b014045f2db4892d2df9334957b8bfd8be7';
...
test('encodeScriptFunctionByResolve6', async () => {
  ...
  const privateKey =
    '0a4a0fe4985df2590ac59c208775f36438a47193ce6eeb197964d8a8f8a6a1f9';
  const STC_SCALLING_FACTOR = 1000000000;
  const addressArray = ['0xFB400ab8753213Cb40c286E740534Ab9'];
  const amountArray = [];
...
test('decoding txn payload', () => {
  const payloadInHex =
    '0x02000000000000000000000000000000010f5472616e73666572536372697074730c706565725f746f5f7065657201070000000000000000000000000000000103535443035354430003101df9157f14b0041eed18dcc56520d82900100060d743dd500b000000000000000000';
...
test('encoding SignedUserTransaction hex, 0x1::DaoVoteScripts::cast_vote', async () => {
  ...
  const config = {
    creator: '0xb2aa52f94db4516c5beecef363af850a',
    id: 1,
    type_args_1:
      '0x1::OnChainConfigDao::OnChainConfigUpdate<0x1::TransactionPublishOption::TransactionPublishOption, d::e::f>',
  };
...
test('decoding SignedUserTransaction hex', () => {
  const hex =
    '';
  const signedUserTransaction = decodeSignedUserTransaction(hex);
  console.log(signedUserTransaction);
  expect(signedUserTransaction.transaction_hash).toBe(
    '0x17d671d18736358b7e6665be3cde9d27a4e515f5f12ce9d014f45ff550be84d3'
  );
  expect(signedUserTransaction.raw_txn.sender).toBe(
    '0x49624992dd72da077ee19d0be210406a'
  );
});
...
test('should convert public key to address correctly', () => {
  ...
  expect(value).toBe('0x400e8f6e15f47c92519e2527fcd64b3a');
});

test('should convert private key to publick key correctly', async () => {
  const privateKey =
    '0xa6d8991ca3d6813f493d13216d6dedd30211a649d21b2ca102b860bea51045fd';
...
test('publicKeyToReceiptIdentifier', () => {
  const publicKey =
    '0x94c3732e3c08eee7738d33b4e6f74daa615da14a94607ac00b531d189cb5b0dd';
  const encodedStrExcepted =
...
test('encode && decode receipt identifier', () => {
  ...
  const authKey =
    '93dcc435cfca2dcf3bf44e9948f1f6a98e66a1f1b114a4b8a37ea16e12beeb6d';
...
test('encode struct type args: lp', () => {
  const strTypeArgs = [
    '0x4783d08fb16990bd35d83f3e23bf93b8::TokenSwap::LiquidityToken<0x00000000000000000000000000000001::STC::STC, 0xfe125d419811297dfab03c61efec0bc9::FAI::FAI>',
  ];
  const structTypeTags = encodeStructTypeTags(strTypeArgs);
  console.log(strTypeArgs);
  console.log(JSON.stringify(structTypeTags, undefined, 2));
...
test('encode/decode data:image/png;base64', () => {
  ...
  const buffer2 = fs.readFileSync(path.join(__dirname, 'data', 'imageHex.txt'));
...
test('encodePackage', async () => {
  ...
  const buffer = fs.readFileSync(path.join(__dirname, 'data', 'TestToken.mv'));
  const fileBytes = new Uint8Array(buffer);
  const fileHex = hexlify(fileBytes);
  const transactionPayload = encodePackage(address, [fileHex], {
