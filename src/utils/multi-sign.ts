 ...
 * @returns
 */
export async function generateMultiEd25519KeyShard(
  originPublicKeys: Array<string>,
  originPrivateKeys: Array<string>,
  thresHold: number
): Promise<MultiEd25519KeyShard> {
  ...
  await Promise.all(
    originPrivateKeys.map((priv) => {
      return privateKeyToPublicKey(priv)
        .then((pub) => {
          publicKeys.push(pub);
          pubPrivMap[pub] = priv;
          return pub;
    ...
    originPrivateKeys.map((priv) => {
      return privateKeyToPublicKey(priv)
        .then((pub) => {
          ...
          if (idx > -1) {
            pos_verified_private_keys[idx] = new Ed25519PrivateKey(
              arrayify(priv)
            );
          }
          return pub;
  ...
  const public_keys = uniquePublicKeys.map(
    (pub) => new Ed25519PublicKey(arrayify(pub))
  );
  const shard = new MultiEd25519KeyShard(
    public_keys,
    thresHold,
...
export async function generateMultiEd25519Signature(
  multiEd25519KeyShard: MultiEd25519KeyShard,
  rawUserTransaction: RawUserTransaction
): Promise<MultiEd25519Signature> {
  const signatures = await Promise.all(
    Object.keys(multiEd25519KeyShard.private_keys).map((k) => {
      ...
      return getSignatureHex(rawUserTransaction, privateKey)
        .then((signatureHex) => {
...
export async function generateMultiEd25519SignatureShard(
  multiEd25519KeyShard: MultiEd25519KeyShard,
  rawUserTransaction: RawUserTransaction
): Promise<MultiEd25519SignatureShard> {
  const multiEd25519Signature = await generateMultiEd25519Signature(
    multiEd25519KeyShard,
    rawUserTransaction
  );
  const multiEd25519SignatureShard = new MultiEd25519SignatureShard(
    multiEd25519Signature,
    multiEd25519KeyShard.threshold
  );
