...
import {
  generateMultiEd25519KeyShard,
  generateMultiEd25519Signature,
  generateMultiEd25519SignatureShard,
} from './multi-sign';
import {
  dec2bin,
  bin2dec,
...
import {
  showMultiEd25519Account,
  decodeMultiEd25519AccountPrivateKey,
} from './account';
...
const alice = {
  ...
  private_key:
    '0xa9e47d270d2ce33b1475f500f3b9a773eb966f3f8ab5ceb738d52262bbe10cb2',
};
...
const bob = {
  ...
  private_key:
    '0x7ea63107b0e214789fdb0d6c6e6b0d8f8b8c0be7398654ddd63f3617282be97b',
};
...
const tom = {
  ...
  private_key:
    '0x359059828e89fe42dddd5f9571a0c623b071379fc6287c712649dcc8c77f5eb4',
};
...
test('import from multiSign account privateKey', async () => {
  ...
  const shardAlice = await generateMultiEd25519KeyShard(
    publicKeys1,
    privateKeys1,
    threshold
  );
  ...
  let shard;
  {
    const {
      ...
      threshold,
      privateKeys,
    } = decodeMultiEd25519AccountPrivateKey(multiAccountAlice.privateKey);
    console.log('decodeMultiEd25519AccountPrivateKey', {
      publicKeys,
      threshold,
      privateKeys,
    });
    shard = await generateMultiEd25519KeyShard(
      publicKeys,
      privateKeys,
      threshold
    );
  ...
  expect(multiAccount.receiptIdentifier).toEqual(
    multiAccountAlice.receiptIdentifier
  );
});
...
test('prepare 3 multi sign accounts', async () => {
  ...
  const shardBob = await generateMultiEd25519KeyShard(
    publicKeys2,
    privateKeys2,
    threshold
  );
  ...
  const shardTom = await generateMultiEd25519KeyShard(
    publicKeys3,
    privateKeys3,
    threshold
  );
...
test('first multi sign', async () => {
  ...
  const args = [receiverAddress, amount];
  ...
  const senderSequenceNumber =
    (await provider.getSequenceNumber(senderAddress)) || 0;
  ...
  const signatureShard = await generateMultiEd25519SignatureShard(
    shardTom,
    rawUserTransaction
  );
  ...
  const partial_signed_txn = new starcoin_types.SignedUserTransaction(
    rawUserTransaction,
    authenticator
  );
  ...
  console.log(`mutlisig txn(address: ${
    multiAccountTom.address
  }, threshold: ${threshold}): ${count_signatures} signatures collected
...
`);
  ...
  try {
    ...
    const signed_txn = bcsDecode(
      starcoin_types.SignedUserTransaction,
      partial_signed_txn_hex
    );
    console.log({ signed_txn });
    expect(partial_signed_txn_hex).toEqual(
      bcsEncode(signed_txn as starcoin_types.SignedUserTransaction)
    );
    console.log({ filename });
...
test('second multi sign', async () => {
  ...
  console.log(hexlify(rbuf));
  const txn: starcoin_types.SignedUserTransaction = bcsDecode(
    starcoin_types.SignedUserTransaction,
    hexlify(rbuf)
  );
  console.log({ txn });
...
threshold= ${threshold}
${count_signatures} signatures collected, still require ${
    threshold - count_signatures
  } signatures
`);

  const { raw_txn } = txn;
  const existingSignatureShards = new starcoin_types.MultiEd25519SignatureShard(
    existingAuthenticator.signature,
    existingAuthenticator.public_key.threshold
  );

  console.log({ raw_txn, existingSignatureShards });
  console.log(
    'existingSignatureShards is_enough',
    existingSignatureShards.is_enough()
  );
  ...
  const mySignatureShards = new starcoin_types.MultiEd25519SignatureShard(
    signatureShard.signature,
    existingAuthenticator.public_key.threshold
  );
  ...
  const mergedSignatureShards = starcoin_types.MultiEd25519SignatureShard.merge(
    signatureShards
  );
  console.log({ mergedSignatureShards });
  console.log(
    'mergedSignatureShards is_enough',
    mergedSignatureShards.is_enough()
  );
  if (!mergedSignatureShards.is_enough()) {
    console.log(
      `still require ${
        mergedSignatureShards.threshold -
        mergedSignatureShards.signature.signatures.length
      } signatures`
    );
  } else {
    console.log(
  ...
  const authenticator = new starcoin_types.TransactionAuthenticatorVariantMultiEd25519(
    shardAlice.publicKey(),
    mergedSignatureShards.signature
  );
...
test('bit operator', () => {
  ...
  console.log({ n });
  const t2 =
    bin2dec('00000000000000000000000000000000') |
    bin2dec('00000000000000000000000000000001') |
    bin2dec('00010000000000000000000000000000');
