...
export function encodeTransactionAuthenticatorEd25519(
  ...
  const authenticatorEd25519 = new TransactionAuthenticatorVariantEd25519(
    ed25519PublicKey,
    ed25519Signature
  );
...
export function getEd25519SignMsgBytes(signingMessage: SigningMessage): bytes {
...
export async function generateSignedMessage(
  ...
  const transactionAuthenticatorEd25519 = encodeTransactionAuthenticatorEd25519(
    signatureBytes,
    publicKeyBytes
  );
...
export async function encodeSignedMessage(
  ...
  const msgBytes = new Uint8Array(Buffer.from(msg, 'utf8'));
  const signingMessage = new SigningMessage(msgBytes);
  const { publicKey, signature } = await signMessage(
    msg,
    hexlify(privateKeyBytes)
  );

  const signedMessageHex = await generateSignedMessage(
    signingMessage,
    chainId,
    publicKey,
    signature
...
export function decodeSignedMessage(data: BytesLike): SignedMessage {
...
export async function recoverSignedMessageAddress(
  ...
  if (
    signedMessage.authenticator instanceof
    TransactionAuthenticatorVariantEd25519
  ) {
